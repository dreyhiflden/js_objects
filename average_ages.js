...
function getCenturiesStats(people) {
  ...
  for (const key in stats) {
    const filtered = people.filter(i => (Math.ceil(i.died / 100)) === parseInt(key, 10));
...
const actualWomen = getAverageWomenAge(ANCESTRY_FILE);
const expectedWomen = 55;
...
const roundedActualDiff = Math.round(getAverageDiff(ANCESTRY_FILE));
const expectedDiff = 31;
